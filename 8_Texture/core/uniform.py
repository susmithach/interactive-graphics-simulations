...
class Uniform(object):
	...
	def __init__(self, dataType, data):
		...
		# type of data
		#	ini | bool | float | vec2 | vec3 | vec4 | sampler2D
	...
	def uploadData(self):
		...
		# when we have more than one element, we need to seperate the elements	
			...
			# load the two types of data from self.data
			textureObjectRef, textureUnitRef = self.data

			# active texture unit
			glActiveTexture( GL_TEXTURE0 + textureUnitRef )

			# associate the texture w/ the texture object
			glBindTexture ( GL_TEXTURE_2D, textureObjectRef)

			# pass the texture unit number to the uniform variable in the shader
